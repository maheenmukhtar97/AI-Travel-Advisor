...
def normalize_city_name(city):
    ...
    if "karachi" in city or city == "khi":
        return "karachi"
    if "lahore" in city or city == "lhe":
        return "lahore"
    if "islamabad" in city or city == "isb":
        return "islamabad"

    return city

def get_route(source, destination):
    source = normalize_city_name(source)
    ...
    try:
        ...
        return {
            ...
            "flight_time": f"{round(duration/5, 2)} hr",   # approx (flight faster)
            "train_time": f"{round(duration*1.5, 2)} hr",
            "bus_time": f"{round(duration*1.2, 2)} hr"
        }
